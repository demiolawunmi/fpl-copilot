...
def _serve_api_file(filename: str) -> FileResponse:
    ...
    # Always revalidate: these snapshots are regenerated by the export pipeline
    # (e.g. my_team.json after a deadline), never serve stale copies from cache.
...
def season_status():
    """Compare the live FPL season against local data snapshots.

    Returns ``{fpl_season, data_season, is_current, checked_at}`` so the UI can
    warn when backend exports (AIrsenal DB / teams.json) still belong to the
    previous season.
    """
    from src.services.current_season import get_season_status
